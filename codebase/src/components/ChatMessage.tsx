...
export function ChatMessage({ message, disabled, onCitation, onSimplify, onPageOnly, onCopy, onLike, onDislike }: Props) {
  ...
  return <div className="flex gap-2.5">
    ...
    <div className="min-w-0 flex-1">
      <div className="rounded-2xl rounded-tl-md border border-slate-200 bg-white px-3.5 py-3 shadow-sm dark:border-slate-700 dark:bg-slate-900">
        ...
        <p className="whitespace-pre-line text-[13px] leading-6 text-slate-700 dark:text-slate-200">{message.content}</p>
      ...
      <div className="mt-1.5 flex flex-wrap items-center gap-1">
        ...
        <button type="button" className="message-icon-action" onClick={() => onCopy(message)} aria-label="Sao chép"><Copy size={13} /></button><span className="mx-0.5 h-4 w-px bg-slate-200 dark:bg-slate-700" />
